...
import com.parent.forecast.models.WeatherViewHolder;
import com.parent.forecast.utils.UnitConvertor;
...
public class WeatherRecyclerAdapter extends RecyclerView.Adapter<WeatherViewHolder> {
    ...
    public void onBindViewHolder(WeatherViewHolder customViewHolder, int i) {
        ...
        try {
            ...
            try {
                date= new Date(Long.parseLong(weatherItem.get_date()) );
            }
            catch (Exception e) {

                SimpleDateFormat inputFormat = new SimpleDateFormat(dateFormat);
                try {
                    date= inputFormat.parse(weatherItem.get_date());
                }
                catch (ParseException e2) {
                    date= new Date(); // make the error somewhat obvious
                    e2.printStackTrace();
                }
            }
        ...
        customViewHolder.itemPressure.setText(context.getString(R.string.pressure) + ": " + new DecimalFormat("#.0").format(pressure) + " " +
                ForecastDetailsActivity.localize(sp, context, "pressureUnit", "hPa"));
